...
router.get('/api/admin_filter_user', async (req, res) => {
    try {
        let users = await DB.User.admin_filter_user(req.query);
        res.json(users);
    } catch (e) {
        console.log(e);
        res.sendStatus(500);
...
router.get('/api/get_filtered_users', async (req, res) => {
    try {
        let users = await DB.Company.get_filtered_users();
        res.json(users);
    } catch (e) {
        console.log(e);
        res.sendStatus(500);
...
router.get('/api/admin_filter_company', async (req, res) => {
    try {
        let companies = await DB.Company.admin_filter_company(req.query);
        res.json(companies);
    } catch (e) {
        console.log(e);
        res.sendStatus(500);
...
router.get('/api/get_filtered_companies', async (req, res) => {
    try {
        let companies = await DB.Company.get_filtered_companies();
        res.json(companies);
    } catch (e) {
        console.log(e);
        res.sendStatus(500);
